...
\end{equation}
%
onde $x_{t+1}$ \'e dado pela equa\c{c}\~ao de transi\c{c}\~ao~\eqref{eq:transicao_caixa}, $\delta_{t,\omega} \geq 0$ \'e a vari\'avel de folga da restri\c{c}\~ao de cr\'edito $x_{t+1} + \delta_{t,\omega} \geq L^{\text{cred}}$, e $M = 10^9$~R\$ \'e o par\^ametro \emph{big-M} escolhido suficientemente grande para tornar qualquer viola\c{c}\~ao de $L^{\text{cred}} = -10^8$~R\$ indesej\'avel frente ao resultado financeiro t\'ipico do portf\'olio.
...
\begin{equation}
  ...
  \mathbb{E}_{\omega_t}\bigl[f_t(y_t, q_t, \omega_t)
  + V_{t+1}(y_{t+1})\bigr],
  \label{eq:funcao_valor}
\end{equation}
%
onde $\mathcal{Q}_t(y_t)$ \'e o conjunto de decis\~oes fact\'iveis. Na pr\'atica, $V_{t+1}(\cdot)$ n\~ao \'e conhecida explicitamente e \'e substitu\'ida por uma aproxima\c{c}\~ao linear por partes $\hat{V}_{t+1}(\cdot)$, constru\'ida iterativamente por cortes de Benders. Cada corte \'e um hiperplano que mant\'em o car\'ater c\^oncavo da fun\c{c}\~ao de valor. A soma telesc\'opica $\sum_{t=1}^{T} f_t = (x_T - x_0) - M\sum_{t=1}^{T}\delta_t$ mostra que o valor acumulado ao longo do horizonte coincide com o resultado financeiro l\'iquido de penalidades do DEQ~\eqref{eq:objetivo_deq}.

\subsection{Algoritmo: Etapas \textit{Forward} e \textit{Backward}}
\label{subsec:forward_backward}

Na etapa \textit{forward}, simula-se a pol\'itica corrente sobre trajet\'orias de realiza\c{c}\~oes $\omega_t$ (PLD e gera\c{c}\~ao f\'isica), registrando os estados visitados $y_t$ e estimando o desempenho da pol\'itica (limite inferior $\underline{z}$). Na etapa \textit{backward}, resolvem-se os subproblemas nos estados visitados e extraem-se as vari\'aveis duais das restri\c{c}\~oes de transi\c{c}\~ao de estado para construir cortes que reduzem a aproxima\c{c}\~ao $\hat{V}_t(\cdot)$ em dire\c{c}\~ao ao valor \'otimo (limite superior $\bar{z}$). O processo itera at\'e $\bar{z} \approx \underline{z}$ --- cf.\ Se\c{c}\~ao~\ref{subsec:cortes_benders} para a conven\c{c}\~ao de maximiza\c{c}\~ao adotada \cite{pereira1991}. Na implementa\c{c}\~ao deste trabalho, o algoritmo executa no m\'aximo 50 itera\c{c}\~oes (\texttt{iteration\_limit = 50}). A principal vantagem em rela\c{c}\~ao ao DEQ \'e que o tamanho de cada subproblema cresce apenas com a dimens\~ao do vetor de estado $y_t$, tornando a PDDE escal\'avel para horizontes longos e \'arvores de grande porte.
