...
A PDDE opera sobre um \emph{grafo de pol\'itica} (\textit{policy graph}), que \'e uma representa\c{c}\~ao mais compacta do espa\c{c}o de decis\~ao do que a \'arvore de cen\'arios completa. Neste trabalho, cada m\^es $t$ do horizonte de planejamento \'e dividido em dois momentos distintos:
...
Essa separa\c{c}\~ao entre o momento de decis\~ao e o momento de realiza\c{c}\~ao das incertezas incorpora naturalmente a n\~ao antecipatividade na pr\'opria estrutura do modelo, sem necessidade de restri\c{c}\~oes expl\'icitas como na formula\c{c}\~ao DEQ. A Figura~\ref{fig:policy_graph} ilustra essa estrutura para um horizonte de tr\^es meses com tr\^es cen\'arios por est\'agio.

\begin{figure}[H]
  \centering
  % Substituir pelo gráfico da implementação quando disponível
  \caption{Estrutura do grafo de pol\'itica utilizado na PDDE: em cada m\^es, a decis\~ao de contrata\c{c}\~ao \'e tomada antes da realiza\c{c}\~ao do cen\'ario, e a liquida\c{c}\~ao ocorre ap\'os a revela\c{c}\~ao das incertezas.}
  \label{fig:policy_graph}
\end{figure}
